...
/**
 * Payments_model - Operations related to Business user log table
 * @author KU
 */
class Payments_model extends CI_Model {
    ...
    /**
     * Get Business users details with last logged in time and count 
     * @param string $type - count or result
     * @return array
     */
    public function get_payments($type = 'result') {
        $columns = ['u.id', 'ui.image', 'u.firstname', 'u.email', 'b.name', 'u.login_count', 'u.last_loggedin'];
        ...
        if (!empty($keyword['value'])) {
            $this->db->where('(u.firstname LIKE "%' . $keyword['value'] . '%" OR u.lastname LIKE "%' . $keyword['value'] . '%" OR u.email LIKE "%' . $keyword['value'] . '%" OR CONCAT(u.firstname," ",u.lastname) LIKE "%' . $keyword['value'] . '%" OR b.name LIKE "%' . $keyword['value'] . '%")');
        }
